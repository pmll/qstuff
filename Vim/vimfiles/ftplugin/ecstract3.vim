...
" these are just sequences of alignment chars, a more generic fn with the
" alignment sequence could be used (i.e. Format(startline, endline, [":", "=", " as "]
function! FormatTable(startLine, endLine)

	let tableLines = getline(a:startLine + 1, a:endLine - 1)
	let colonAligned = AlignList(tableLines, ":")
	let equalAligned = AlignList(colonAligned, "=")
	let asAligned = AlignList(equalAligned, " as ")
	call setline(a:startLine + 1, asAligned)

endfunction


function! FormatVar(startLine, endLine)

	let varLines = getline(a:startLine + 1, a:endLine - 1)
	let colonAligned = AlignList(varLines, ":")
	let asAligned = AlignList(colonAligned, " as ")
	call setline(a:startLine + 1, asAligned)
